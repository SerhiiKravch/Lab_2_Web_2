...
async function server_PUT (req, array) {
   ...
   switch (req) {
      ...
      case "/set_performers":       collection = 3; break;
...
function save_data_in_local_storage() {
   ...
   switch (target) {
      ...
      case "performers":
         localStorage.setItem('performers', JSON.stringify(get_performers_list()));
         break;
   ...
   let identificators = [{ "name":"last_project_id","value":last_project_id },
                         ...
                         { "name":"last_performer_id","value":last_performer_id }];
...
function save_data_in_data_base() {
   ...
   switch (target) {
      ...
      case "performers":
         server_PUT("/set_performers", get_performers_list());
         break;
   }
...
async function load_data_from_local_storage() {
   ...
   switch (target) {
      ...
      case "performers":
         item = JSON.parse(localStorage.getItem("performers"));
         set_performers_list(item ? item : []);
         break;
   ...
   for (let item of identificators) {
      ...
      if (item.name === "last_performer_id")  { last_performer_id  = item.value; }
   }
}
...
async function load_data_from_data_base() {
   ...
   switch (target) {
      ...
      case "performers":
         await server_GET("/get_performers").then((res) =>
            { set_performers_list(res); });
         break;
   ...
   await server_GET("/get_last_performer_id").then((res) =>
      { if (res && res.length > 0) { last_performer_id = res[0].value; }});
